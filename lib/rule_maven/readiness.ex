defmodule RuleMaven.Readiness do
  ...
  **required** and together define `games.playable` (the new, stronger
  ...
  @enrichment ~w(theme suggestions categories cheat_sheet setup did_you_know voices first_player common_mistakes quiz teach score turn)a

  # Steps that spend LLM/embedding budget — the ones we estimate + total cost for.
  @llm_steps ~w(extract cleanup embed suggestions categories cheat_sheet setup did_you_know voices theme first_player common_mistakes quiz teach score turn)a
  ...
  def label(:score), do: "Score pad"
  def label(:turn), do: "Turn wizard"
  ...
  def step_complete?(:score, %Game{id: id}, _docs), do: present?("score_categories_#{id}")
  def step_complete?(:turn, %Game{id: id}, _docs), do: present?("turn_flow_#{id}")
  ...
  def ensure_enrichments(%Game{} = game) do
    ...
    unless step_complete?(:score, game, []),
      do: RuleMaven.Workers.ScoreCategoriesWorker.enqueue(game.id)

    unless step_complete?(:turn, game, []),
      do: RuleMaven.Workers.TurnFlowWorker.enqueue(game.id)
