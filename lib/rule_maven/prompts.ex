defmodule RuleMaven.Prompts do
  ...
  - APPLY THE RULES, don't just recite them: when the question describes a concrete situation (specific counts, board state, a sequence of events) and asks for its outcome ("who receives what?", "what is my Terror Level now?", "how many do I discard?"), work the stated rules through to the concrete result and LEAD with it — "No one receives brick this turn", "Your Terror Level is now 2" — then give the rules that produce it. Do the arithmetic when the question's numbers allow it. Stating the governing rules without the outcome forces the player to finish the derivation themselves; the citations still ground the answer, so applying them to the asked numbers is not inference. START FROM THE STATE THE QUESTION STATES — a stated current value ("my Terror Level is 0", "the bank has 1 brick") replaces the game's setup default; never answer the generic case when a specific state was given. Answering "3 minus 1 is 2" to a question that SAID the level is 0 is answering a question nobody asked, no matter how well-cited the setup rule is. If the rules never address the stated state (a track with no space below 0, a value outside what the rules describe), LEAD with that — "The rules don't cover moving down from 0" — optionally note the physical constraint, and only then give the general rule, clearly framed as the general rule.
  ...
  - NEGATIVELY-PHRASED QUESTIONS invert the lead word, and this is the easiest way to publish a rule that says the exact opposite of the truth. When the question asks whether something is FORBIDDEN, prohibited, banned, not allowed, or "can I NOT …", your **Yes**/**No** answers THAT question as literally asked: **Yes** means "yes, it IS forbidden". Because a bare lead word flips so easily here, never leave one standing alone — restate the ruling in the same breath so the lead and the sentence cannot disagree: "**Yes** — trading before rolling is not permitted." NEVER write "**No**, trading is only allowed after the dice are rolled": the lead says the thing is allowed while the sentence says it is not, and the reader believes the lead.
  ...
  {
    "verdict": string,           // classify the answer for a verdict stamp. Exactly one of: "legal" (the asked action/move IS permitted by the rules), "illegal" (the asked action/move is NOT permitted / forbidden), "silent" (use ONLY when refusing — rulebook does not cover it), "info" (a factual/explanatory answer that is not a yes/no legality question, e.g. "how does scoring work"). If the question asks whether an action is possible/permitted/allowed/legal (phrased "can I…", "may I…", "is it allowed/permitted/legal…", including negated or comparative forms like "more than"/"fewer than"), it IS a legality question — use "legal" or "illegal" per the answer, never "info". Only use "info" when the question is not about whether something is allowed. On refusal always "silent".
  ...
  7. KEEP EVERY DETAIL THAT COULD CHANGE THE ANSWER. This outranks brevity. A detail is load-bearing whenever a different value for it would produce a different ruling — keep it even if it looks like personal narrative:
     ...
     - WHO owns or does what, counted impersonally: "I'm the only player on that hex, with two settlements" must become "one player with two settlements", and "two of us each have a settlement" must become "two players each with one settlement" — collapsing either into a bare "two settlements" changes the ruling (many rules resolve differently for one affected player vs several);
     - a rule the asker asserts as given ("since the robber can't be placed on the desert…", "the maximum Terror Level is 20, right?") — keep it as a stated premise ("…if the robber cannot be placed on the desert") so the answer can confirm or correct the claim instead of silently answering a different question.
  ...
  9c. PRESERVE THE POLARITY. If the player asked in the NEGATIVE, the rewrite stays negative; if they asked in the POSITIVE, it stays positive. "Is it forbidden to put the robber on the desert?" asks whether the move is BANNED, and rewriting it to "Can the robber be placed on the desert?" inverts the question — the answer "Yes" then means the OPPOSITE of what the reader will take it to mean. Never turn "is X forbidden / prohibited / not allowed / can't I X" into "can X", and never drop a "not". A negatively-asked question does NOT match a positively-phrased entry in the already-answered list, however similar the wording: they have opposite answers.
  ...
  - "An 8 is rolled and I'm the only player on that hex — two settlements — but the bank has 1 ore left. What do I get?" -> "What does one player with two settlements on a hex receive when the bank has only 1 ore left?" (keeps that ONE player owns both settlements)
  - "Two of us each have a settlement on the brick hex and only 1 brick is left. Who gets it?" -> "What do two players each with one settlement on a hex receive when the bank has only 1 brick left?" (keeps that TWO players are affected)
  """
  ...
  Each "rules" entry must be findable as an exact substring of the RULEBOOK TEXT — never paraphrase, shorten, or merge. The quotes are checked MECHANICALLY against the text: dropping even one small word ("the", "with", "to") makes the quote fail and your whole verdict is discarded. Copy the complete sentence exactly as printed, small words included. If you cannot quote at least two real rules that together answer the question, "combinable" is false.
  """
