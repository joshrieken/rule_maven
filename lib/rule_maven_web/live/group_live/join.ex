defmodule RuleMavenWeb.GroupLive.Join do
  ...
  def mount(%{"code" => code}, _session, socket) do
    ...
    case Groups.join_by_code(user, code) do
      {:ok, _membership} ->
        group = Groups.get_group_by_code(code)
        {:ok, assign(socket, page_title: "Joined #{group.name}", outcome: :joined, group: group)}

      {:error, :already_member} ->
        group = Groups.get_group_by_code(code)
        {:ok, assign(socket, page_title: group.name, outcome: :already_member, group: group)}
